...
const BrandLogo = ({ className, imageClassName, labelClassName, showLabel = true }: BrandLogoProps) => {
  return (
    <div className={cn("flex items-center gap-2", className)}>
      ...
      />
      {showLabel ? <span className={cn("font-body text-foreground", labelClassName)} style={{ fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.12em", fontSize: "1rem" }}>Polarist</span> : null}
    </div>
  );
};
